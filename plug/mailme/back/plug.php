...
class PlugMailme extends Plug{
	...
	public $plugName = 'mailme';
	public $plugNameRu = 'Написать письмо';
	...
	{
		#table plug_mail_history
		$res = $this->db->query("SELECT count(*) FROM `plug_mailme_history`");
		if(!$res)
		{
			$this->db->query("
				CREATE TABLE IF NOT EXISTS `plug_mailme_history` (
				  ...
				  `request_departament` varchar(255) NOT NULL COMMENT 'departament name',
				  `request_name` varchar(255) NOT NULL COMMENT 'user name',
				  `request_email` varchar(255) NOT NULL COMMENT 'user phone',
				  `request_text` text NOT NULL COMMENT 'text',
				  PRIMARY KEY (`id`)
				) ENGINE=MyISAM DEFAULT CHARSET=cp1251 AUTO_INCREMENT=1 ;
			");
		}	

		#table plug_mailme_departaments
		$res = $this->db->query("SELECT count(*) FROM `plug_mailme_departaments`");
		if(!$res)
		{
			$this->db->query("
				CREATE TABLE IF NOT EXISTS `plug_mailme_departaments` (
				  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'just id',
				  `name` varchar(255) NOT NULL COMMENT 'departamrnt name',
				  PRIMARY KEY (`id`)
				) ENGINE=MyISAM DEFAULT CHARSET=cp1251 AUTO_INCREMENT=1 ;
			");
		}	

		#table plug_mailme_recipient_email
		$res = $this->db->query("SELECT count(*) FROM `plug_mailme_recipient_email`");
		if(!$res)
		{
			$this->db->query("
				CREATE TABLE IF NOT EXISTS `plug_mailme_recipient_email` (
				  `email` varchar(255) NOT NULL COMMENT 'recipient email address',
				  `name` varchar(255) NOT NULL COMMENT 'recipient persone name',
				  `departament_id` int(11) NOT NULL DEFAULT '0' COMMENT 'departament id',
				  UNIQUE KEY `email` (`email`)
				) ENGINE=MyISAM DEFAULT CHARSET=cp1251;
			");
		}	

		#table plug_mailme_sett
		$res = $this->db->query("SELECT count(*) FROM `plug_mailme_sett`");
		if(!$res)
		{
			$this->db->query("
				CREATE TABLE IF NOT EXISTS `plug_mailme_sett` (
			...
			$this->db->query("INSERT INTO `plug_mailme_sett` (`api_id`) VALUES ('');");
		}	

		#add new words
		$res = $this->db->query("SELECT `id` FROM `words` WHERE `word_key`='plug_mailme_message_ok'");
		$resArray = $res->fetch();
		if(empty($resArray))
		{
			$this->db->query("
				INSERT INTO `words` SET `word_key`='plug_mailme_message_ok', `word_value`='Спасибо. Ваша заявка принята.', `word_desc`='Написать письмо. Сообщение успешно принятой заявки.'
			");
		}
	...
	public function ajax( $postArray = array() )
	{
		...
		switch( $postArray['action'] )
		{
			case 'history':
				#get history
				$res = $this->db->query("SELECT * FROM `plug_mailme_history` ORDER BY `request_date`DESC,`request_time`DESC");
			...
			case 'clearHistory':
				$this->db->query("DELETE FROM `plug_mailme_history`");
			break;
			...
			case 'recipient_email_tpl':
				#get recipients
				$res = $this->db->query("SELECT * FROM `plug_mailme_recipient_email`");
				$resArray = $res->fetchAll();
				if( !empty($resArray) ) $this->template->assign('recipients', $resArray);
				
				#get recipients
				$res = $this->db->query("SELECT * FROM `plug_mailme_departaments`");
				$resArray = $res->fetchAll();
				if( !empty($resArray) )
				{
					$this->template->assign('departamentsList', json_encode($resArray));
					$this->template->assign('departaments', $resArray);
				}else{
					$this->template->assign('departamentsList', json_encode(array()));
				}
			...
			case 'departaments_tpl':
				#get recipients
				$res = $this->db->query("SELECT * FROM `plug_mailme_departaments`");
				$resArray = $res->fetchAll();
				if( !empty($resArray) ) $this->template->assign('departaments', $resArray);
				$this->template->display('departaments_tpl.tpl');
			break;
			...
			case 'departamentsSave':
				unset($postArray['action']);
				#clear all data
				$this->db->query("DELETE FROM `plug_mailme_departaments`");
				...
				if( !empty($postArray) )
				{
					...
					{
						$queryValues[] = "('".$data['name']."')";
					}
					$this->db->query("INSERT INTO `plug_mailme_departaments` (`name`) VALUES ".implode(",", $queryValues));
				}
			break;
			...
			case 'recipientEmailSave':
				unset($postArray['action']);
				#clear all data
				$this->db->query("DELETE FROM `plug_mailme_recipient_email`");
				...
				if( !empty($postArray) )
				{
					...
					foreach($postArray as $data)
					{
						$queryValues[] = "('".$data['email']."', '".$data['name']."', '".$data['departament_id']."')";
					}
					//echo "INSERT INTO `plug_mailme_recipient_email` (`email`, `name`, `departament_id`) VALUES ".implode(",", $queryValues);
					$this->db->query("INSERT INTO `plug_mailme_recipient_email` (`email`, `name`, `departament_id`) VALUES ".implode(",", $queryValues));
				}
			break;
