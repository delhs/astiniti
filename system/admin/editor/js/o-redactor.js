	...
	Redactor.fn = $.Redactor.prototype = {
		...
		pickerSet: function(rule, type)
		{
			...
			if (type !== false){
				...
				this.insertHtml('<span data-redactor="verified" style="'+rule+':'+type+'">'+str+'</span>');

				//this.inlineFormat('span');
		...
		{
			var $el = $(e);
			var parent = $el.parents(".redactor-imagebox-edit:first").parent();

			var callback = $.proxy(function()
			{
			
				//$("#redactor_modal").off("change.align");
				$("#redactor_form_image_align").change(function(){
					if( $(this).val()=="center" ){
						$("#redactor_file_margin_top, #redactor_file_margin_right, #redactor_file_margin_bottom, #redactor_file_margin_left").attr("readonly", "readonly");
					}else{
						$("#redactor_file_margin_top, #redactor_file_margin_right, #redactor_file_margin_bottom, #redactor_file_margin_left").removeAttr("readonly", "readonly");
					}	
				});



				$('#redactor_file_width').val( $el[0].style.width );
				$('#redactor_file_height').val( $el[0].style.height );
				
				
				$('#redactor_file_margin_top').val($el.parent(".ui-wrapper")[0].style.marginTop);
				$('#redactor_file_margin_right').val($el.parent(".ui-wrapper")[0].style.marginRight);
				$('#redactor_file_margin_bottom').val($el.parent(".ui-wrapper")[0].style.marginBottom);
				$('#redactor_file_margin_left').val($el.parent(".ui-wrapper")[0].style.marginLeft);
				
				$('#redactor_file_alt').val($el.attr('alt'));
				
				if( $el.attr('fancy')!=undefined ) $('#redactor_file_fancy').attr("checked", "checked");
				
				$('#redactor_file_title').val($el.attr('title'));
				
				$('#redactor_image_edit_src').attr('href', $el.attr('src'));

				if(    $el[0].style.marginTop == "0px"
					&& $el[0].style.marginBottom == "0px"
					&& $el[0].style.marginLeft == "auto"
					&& $el[0].style.marginRight == "auto"
					&& $el[0].style.display=='block' 
					&& ($el[0].style.float=='none' || $el[0].style.float==undefined )
				   ){
				...
				if( $el.parents(".ui-wrapper:first").get(0).style.margin == "0px auto"){
					$('#redactor_form_image_align').val("center");
					$("#redactor_file_margin_top, #redactor_file_margin_right, #redactor_file_margin_bottom, #redactor_file_margin_left").attr("readonly", "readonly");
				}
		...
		imageSave: function(el)
		{
			...
			$(el).attr('title', $('#redactor_file_title').val());
			
			
			var cssWidth = $('#redactor_file_width').val();
			
			var cssHeight = $('#redactor_file_height').val();
			
			var cssMargin = $('#redactor_file_margin_top').val()+' '+$('#redactor_file_margin_right').val()+' '+$('#redactor_file_margin_bottom').val()+' '+$('#redactor_file_margin_left').val();
			
			var display = 'inline';

			if( cssMargin=='0px 0px 0px 0px' ){
				cssMargin = '0';
			}
			
			
			var floating = $('#redactor_form_image_align').val();
			floating = ( floating!='none' ) ? floating : 'none';
			if( floating=='center' ){
				cssMargin = '0px auto';
				floating = 'none';
				display = 'block';
			...
			//$(el).css({'float':floating, 'width':cssWidth, 'height':cssHeight, 'margin':cssMargin, 'display':display});
			$(el).css({'width':cssWidth, 'height':cssHeight, 'float':floating});
			$(el).parent(".ui-wrapper").css({'width':cssWidth, 'height':cssHeight, 'margin':cssMargin});
			$(el).parents(".redactor-imagebox-edit").css({'float':floating, 'display':display});

			if( $('#redactor_file_fancy').prop("checked") ){
				$(el).attr("fancy", "on");
			}else{
				$(el).removeAttr("fancy");
			}
			...
			if( link !=='' ){
				...
				if ( !$(el).parents(".redactor-imagebox-edit").parent("a").length ){
					var a = $(document.createElement("a")).attr("href", link);
					$(el).parents(".redactor-imagebox-edit").wrap(a);
				}
				else{
					$(el).parents(".redactor-imagebox-edit").parent("a").attr('href', link);
				}
			}else{
				
				if ($(el).parents(".redactor-imagebox-edit").parent("a").length){
					$(el).parents(".redactor-imagebox-edit").unwrap();
				}
			}
		...
		modalTemplatesInit: function()
		{
			$.extend( this.opts,
			{
				...
				+ '<section>'
					...
					+ '<label>Ширина(px или %)</label>'
					+ '<input type="text" id="redactor_file_width" class="redactor_input" />'
					
					+ '<label>Высота(px или %)</label>'
					+ '<input type="text" id="redactor_file_height" class="redactor_input" />'
					
					+ '<label>Отступ свурху(px или %)</label>'
					+ '<input type="text" id="redactor_file_margin_top" class="redactor_input" />'
					
					+ '<label>Отступ справа(px или %)</label>'
					+ '<input type="text" id="redactor_file_margin_right" class="redactor_input" />'
					
					+ '<label>Отступ снизу(px или %)</label>'
					+ '<input type="text" id="redactor_file_margin_bottom" class="redactor_input" />'
										
					+ '<label>Отступ слева(px или %)</label>'
					+ '<input type="text" id="redactor_file_margin_left" class="redactor_input" />'
					
					+ '<label class="redactor_label_block">Тег ALT</label>'
					+ '<input type="text" id="redactor_file_alt" class="redactor_input" />'
					
					+ '<label class="redactor_label_block">Тег TITLE</label>'
					+ '<input type="text" id="redactor_file_title" class="redactor_input" />'
					
					+ '<label class="redactor_label_checkbox">Увеличивать по клику (разрешить Fancybox)</label>'
					+ '<input id="redactor_file_fancy" type="checkbox" class="redactor_input redactor_checkbox" value="1" />'
					
					
					+ '<label class="redactor_label_block">Ссылка</label>'
					+ '<input type="text" id="redactor_file_link" class="redactor_input" />'
					
					+ '<label class="redactor_label_block">Выравнивание (обтекание текстом)</label>'
				...
				+ '<section>'
					+ '<div id="redactor_tabs" style="display:none">'
						+ '<a href="#">' + this.opts.curLang.upload + '</a>'
				...
				+ '<section>'
					+ '<form id="redactorInsertLinkForm" method="post" action="">'
						...
						+ '<div class="redactor_tab" id="redactor_tab1">'
							...
							+ '<div class="clear"></div>'
							+ '<label for="redactor_link_blank"><input type="checkbox" id="redactor_link_blank"> ' + this.opts.curLang.link_new_tab + '</label>'
						+ '</div>'
...
RedactorPlugins.imageEdit = {
	...
	init: function(){
		...
		$(document).on("click.imageeditmode", function(e){
			...
			if( $(e.target).closest("#redactor_modal").length ){return};
	...
	setEvent:function( image ){
		...
		//$image.data('marginLeft', $image[0].style.marginLeft);
		//$image.data('marginTop', $image[0].style.marginTop);
		//$image.data('marginRight', $image[0].style.marginRight);
		//$image.data('marginBottom', $image[0].style.marginBottom);
	...
	unsetEvent:function( callback ){
		...
		$(that.$editor).find(".redactor-imagebox-edit").find("img").each(function(){
			var $uiWrapper = $(this).parent(".ui-wrapper");
			var $redactorImageboxEdit = $(this).parents(".redactor-imagebox-edit");
			if( $uiWrapper.get(0).style.margin=="0px auto" && $uiWrapper.get(0).style.display=="block"){
				$(this).css({margin:"0px auto", display:"block", float:"none"});
			}else{
				$(this).css( {marginRight: $uiWrapper.get(0).style.marginRight } );
				$(this).css( {marginLeft: $uiWrapper.get(0).style.marginLeft } );
				$(this).css( {marginTop: $uiWrapper.get(0).style.marginTop } );
				$(this).css( {marginBottom: $uiWrapper.get(0).style.marginBottom } );
				$(this).css( {float: $redactorImageboxEdit.get(0).style.float } );
				$(this).css( {display: $redactorImageboxEdit.get(0).style.display } );
			}	
...
RedactorPlugins.fontfamily = {
	init: function ()
	{
		var fonts = [ 'Arial', 'Helvetica', 'Georgia', 'Times New Roman', 'Monospace', 'Anonymous Pro', 'Azoft Sans' ];
		var that = this;
		var dropdown = {};
	...
	{
		var string = this.getSelectionText();
		this.insertHtml('<span data-redactor="verified" style="font-family:'+value+'">'+string+'</span>');



		//..
		//this.inlineWrap('span');
...
RedactorPlugins.fontsize = {
	init: function()
	{
		var fonts = [8, 10, 11, 12, 14, 16, 18, 20, 24, 28, 30, 40, 45, 50, 55, 60, 65, 70, 75, 90, 100];
		var that = this;
		var dropdown = {};
...
RedactorPlugins.fullscreen = {
	init: function()
	{
		...
		this.buttonAdd('fullscreen', 'Развернуть на весь экран', $.proxy(this.toggleFullscreen, this));
	...
	toggleFullscreen: function()
	{
		...
		if (!this.fullscreen)
		{
			this.buttonChangeIcon('fullscreen', 'Вернуть в первоначальное состояние');
			...
			$( this.$editor ).next().next().css({height:  ( $(window).height() - $(this.$editor).find(".redactor_toolbar").outerHeight(true)-20  )+"px"});
			$( this.$editor ).next().next().find(".CodeMirror-gutters").css({height: $(window).height()  + "px"});
			$( this.$editor ).parent(".redactor_box").children(".ui-resizable-se").hide();
		...
		else
		{
			...
			$( this.$editor ).next().next().css({height:  ( height - $(this.$editor).find(".redactor_toolbar").outerHeight(true)-20  )+"px"});
			$( this.$editor ).next().next().find(".CodeMirror-gutters").css({height: height  + "px"});
