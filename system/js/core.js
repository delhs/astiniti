...
var core = {
	...
	init:function(){
		...
		var cats = document.querySelectorAll("thecat");
	...
	fancyBoxLoad:function(){
		...
		if( window.editedFromAdmin==undefined ){
			//get all images and wrap them
			var fancyset = document.querySelectorAll("img[data-fancy='on']"); 
			for( var i=0;i<fancyset.length;i++ ){
				//check for fancy exist
				var parent = fancyset[i].parentNode;
				if( parent.tagName=='A' && parent.getAttribute('rel')=='tofancy' ) return;
				var src = fancyset[i].src;

				//wrap
				a = document.createElement('a');
				a.setAttribute('href', src);
				a.setAttribute('rel', 'tofancy');
    			fancyset[i].parentNode.insertBefore(a, fancyset[i]);
    			a.appendChild(fancyset[i]);
			}
		}
